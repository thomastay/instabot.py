...
class InstaBot:
    ...
    def post_auto(self):
        ...
        toPost = self.posts_db_c.fetchone()
        ...
        api = InstagramAPI(self.user_login, self.user_password)
        api.login()  # login
        isUploaded = api.uploadPhoto(filename, caption=captionStr)
        self.posts_db_c.execute("UPDATE posts SET post_date = '2018-08-14' WHERE filename = '{0}'".format(toPost[0]))
